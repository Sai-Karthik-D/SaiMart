  ...
  const placeOrder = async () => {
    try {
      if (!selectedAddress || !selectedAddress._id) {
        return toast.error("Please select a valid address before placing the order.");
      }
  ...
  return products.length > 0 && cartItems ? (
    <div className="flex flex-col md:flex-row mt-16">
      ...
      <div className="flex-1 max-w-4xl">
        ...
        {cartArray.map((product, index) => (
          ...
          >
            ...
            <button onClick={() => removeFromCart(product._id)} className="cursor-pointer mx-auto">
              <img src={assets.remove} alt="remove" className="inline-block w-6 h-6" />
            </button>
          </div>
        ))}
        ...
        >
          <img className="w-8 h-4 group-hover:-translate-x-1 transition" src={assets.arrow_r} alt="arrow" />
          Continue Shopping
        </button>
      </div>
